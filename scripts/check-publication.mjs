...
try {
 ...
 try {
  ...
  globalThis.fetch=async (url,options)=>{
   ...
   assert.ok(String(url).includes('api.groq.com'),'Successful Groq rewriting must not call feeds or another provider');
   submitted=JSON.parse(JSON.parse(options.body).messages[1].content);
