...
export function Sources({statuses=[]}: {statuses?:SourceStatus[]}) {
 ...
 return <div className="sources-footer"><MorphingModal viewId={view} onClose={()=>setView(null)} title={selected?.name??'Behind the patch notes'} trigger={
  <button type="button" className="sources-trigger" onClick={()=>setView('all')}><span className="favicon-stack">{sources.slice(0,5).map(source=><Favicon key={source.name} {...source}/>)}</span><span><strong>{sources.length} sources</strong><span className="sources-caption">Independent reporting. Planetary perspective.</span></span><span className="sources-open" aria-hidden="true">↗</span></button>
 }>
