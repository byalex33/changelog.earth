...
Use 3–7 words. Describe a game mechanic: spawn, respawn, skill unlock, map expansion, stat buff, repair, quest or balance change. Use sentence case, no ending punctuation and no change marker.
...
export function applyPatchTitles(articles, output) {
 ...
 return articles.map((article,id)=>({...article,title:titles.get(id),titleRevision:1}));
}
...
export async function writePatchTitles(articles,{apiKey='',model='gemini-3.5-flash',hfToken='',hfModel='Qwen/Qwen3.5-35B-A3B',fetcher=fetch}={}) {
 ...
 try {
  ...
  return applyPatchTitles(articles,JSON.parse(c.message.content));
 }
}
