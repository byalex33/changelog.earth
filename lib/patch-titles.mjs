...
import { reviewPatchTitles } from './jev.mjs';
...
Research and proposals can qualify under the worldwide policy: an experimental limit is a wording constraint, not itself a reason to exclude a story. Examples: "Possible cause found for long COVID fatigue debuff", "New penguin joins the known species roster", "Mouse stroke debuff reversed with stem cells". For excluded stories, still write a faithful title about the news, never a moderation verdict such as "study excluded".
...
export async function writePatchTitles(articles,options={}) {
 ...
 return options.gatewayApiKey ? reviewPatchTitles(written,options) : written;
}
