...
export async function getPublishedChangelog({draft = false} = {}) {
 ...
 if (!draft) return {articles:archive,editorial:'archived',checkedAt:Date.now(),unavailable:[],stale:[],sourceStatus:[]};
 ...
 const pending = archive.filter(a=>a.titleStyleVersion!==2 || !['gemini','huggingface'].includes(a.titleProvider));
 if (pending.length) {
