...
export function mergeEditions(previous, additions) {
 ...
 for (const article of additions) {
  ...
  if (urls.has(article.url)) {
   ...
   if ((article.titleRevision ?? 0)>(result[index].titleRevision ?? 0)) result[index]={...result[index],title:article.title,titleRevision:article.titleRevision,titleProvider:article.titleProvider,titleModel:article.titleModel,titleStyleVersion:article.titleStyleVersion};
   continue;
  }
...
export function validateArchive(articles) {
 ...
 for (const article of articles) {
  ...
  if (article.titleProvider !== undefined && !['gemini','huggingface'].includes(article.titleProvider)) throw new Error('Invalid title provider');
