...
export function mergeEditions(previous, additions) {
 ...
 for (const article of previous) {
  ...
  const day = article.date.slice(0,10); if (article.worldwide !== false) days.set(day,(days.get(day) ?? 0)+1);
 }
 for (const article of additions) {
  ...
  if (urls.has(article.url)) {
   ...
   if ((article.titleRevision ?? 0)>(result[index].titleRevision ?? 0)) {
    days.set(day,(days.get(day) ?? 0) + Number(article.worldwide !== false) - Number(result[index].worldwide !== false));
    result[index]={...result[index],title:article.title,note:article.note,kind:article.kind,titleRevision:article.titleRevision,titleProvider:article.titleProvider,titleModel:article.titleModel,titleStyleVersion:article.titleStyleVersion,worldwide:article.worldwide,scopeReason:article.scopeReason,jev:article.jev ?? result[index].jev};
   }
   continue;
  }
  if (titles.has(title) || (article.worldwide !== false && (days.get(day) ?? 0)>=6)) continue;
  result.push(article); urls.add(article.url); titles.add(title); if (article.worldwide !== false) days.set(day,(days.get(day) ?? 0)+1);
 }
...
export function validateArchive(articles) {
 ...
 for (const article of articles) {
  ...
  if (article.titleProvider !== undefined && !['gemini','huggingface','groq','editorial'].includes(article.titleProvider)) throw new Error('Invalid title provider');
  ...
  if (article.worldwide !== undefined && typeof article.worldwide !== 'boolean') throw new Error('Invalid worldwide assessment');
  if (article.scopeReason !== undefined && (typeof article.scopeReason !== 'string' || !article.scopeReason.trim() || article.scopeReason.length>400)) throw new Error('Invalid scope reason');
  if (article.jev !== undefined && (!article.jev || article.jev.model!=='typesafe-ai/jev' || !['eligibleProbability','faithfulProbability'].every(key=>Number.isFinite(article.jev[key]) && article.jev[key]>=0 && article.jev[key]<=1))) throw new Error('Invalid Jev assessment');
 }
 return articles;
}
