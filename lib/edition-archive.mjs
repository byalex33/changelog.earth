 ...
 for (const article of additions) {
  ...
  if (urls.has(article.url)) {
   ...
   if ((article.titleRevision ?? 0)>(result[index].titleRevision ?? 0)) {
    ...
    result[index]={...result[index],title:article.title,note:article.note,kind:article.kind,titleRevision:article.titleRevision,titleProvider:article.titleProvider,titleModel:article.titleModel,titleStyleVersion:article.titleStyleVersion,worldwide:article.worldwide,scopeReason:article.scopeReason};
   }
   continue;
  }
