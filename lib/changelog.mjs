...
export async function getChangelog(news, {apiKey = '', model = 'gemini-3.5-flash', fetcher = fetch, now = Date.now()} = {}) {
 const fallback = {...news, articles:news.articles.slice(0, 6), editorial:'unavailable'};
 if (!apiKey || !news.articles.length) return {...fallback, editorial:apiKey ? 'empty' : 'unconfigured'};
 ...
 if (pending) { await pending; return getChangelog(news, {apiKey,model,fetcher,now}); }
 ...
 pending = (async () => {
  try {
   ...
   const edition = {articles:validateEdition(JSON.parse(text),news.articles),editorial:'generated'};
