...
function selectionInput(articles,saved) {
 return JSON.stringify({candidates:articles.map(({title,category,date},sourceId)=>({sourceId,title,category,date:date.slice(0,10)})),previouslySelected:saved.filter(article=>articles.some(candidate=>candidate.date.slice(0,10)===article.date.slice(0,10))).map(({originalTitle,title,date})=>({title:originalTitle ?? title,date:date.slice(0,10)}))});
}
