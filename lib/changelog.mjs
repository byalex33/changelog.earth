...
export async function getChangelog(news, {apiKey = '', model = 'openai/gpt-oss-120b', fetcher = fetch, now = Date.now(), archive = [], strict = false} = {}) {
 ...
 news = {...news,articles:news.articles.filter(article=>!urls.has(article.url) && (counts.get(article.date.slice(0,10)) ?? 0)<6).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,24)};
...
function selectionInput(articles,saved) {
 return JSON.stringify({candidates:articles.map(({title,summary,category,date},sourceId)=>({sourceId,title,summary:(summary ?? "").slice(0,900),category,date:date.slice(0,10)})),previouslySelected:saved.filter(article=>Date.parse(article.date)>=Math.min(...articles.map(candidate=>Date.parse(candidate.date)))-7*86_400_000).map(({originalTitle,title,date})=>({title:originalTitle ?? title,date:date.slice(0,10)}))});
}
