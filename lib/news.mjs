...
export async function getNews(fetcher = fetch, now = Date.now()) {
 ...
 pending = (async () => {
  ...
  const results = await Promise.allSettled(sources.map(async source => {
   ...
   const response = await fetcher(source.url, {signal:AbortSignal.timeout(8_000),headers:{Accept:rss ? 'application/rss+xml, application/atom+xml, application/xml, text/xml' : 'application/json'}});
